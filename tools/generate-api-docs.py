...
SOURCEBASE = "https://raw.githubusercontent.com/EdgeTX/edgetx/main/radio/src/lua/"
...
def generateFunctionDoc(f):
  ...
  # doc = "<!-- This page was build by the script on %s. Do not edit it, any changes will be lost! -->\n\n" % datetime.datetime.utcnow().strftime('%Y/%m/%d %H:%M:%S UTC')
...
for m in MODULES.iterkeys():
  ...
  for f in sorted(MODULES[m]):
    ...
    # print(doc)
    docName = "lua_api_reference/%s-functions-less-than-greater-than-luadoc-begin-%s/%s.md" % (f[0], f[0], f[1].lower())
